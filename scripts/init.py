...
def addEntity(pos, color, scale):
    ...
    g_GraphicsCollection.AddComponent(g_GraphicsFactory())
    #print(gComp_cptr)
...
def createOrthoCamera(left, right, bottom, top, near, far):
    g_Camera.InitOrtho(left, right, bottom, top, near, far)
...
def initShader():
    handleShaderErrs(b'../Shaders/simple.vert', b'../Shaders/simple.frag')